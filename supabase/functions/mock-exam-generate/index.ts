...
// Korean TTS Voices - 영구 저장 (사용자 지정)
const ELEVENLABS_VOICES = {
  // 여자 목소리
  female: "ksaI0TCD9BstzEzlxj4q",
  // 남자 목소리
  male: "bciERhbhQhAIWwvnQA7H",
} as const;
...
## [51번] 실용문 빈칸 완성 (10점)
**형식**: 짧은 실용문(감사문, 안내문, 초대문, 공지 등)에서 2개의 빈칸(ㄱ, ㄴ)을 채우기
**글자 수**: 각 빈칸 약 20~30자 내외
**톤**: 존댓말(~습니다/~ㅂ니다) 사용

**예시 (83회 기출)**:
\`\`\`
[감사의 글]
저는 10년 전에 이 장학금을 받았습니다. 그때 저희 가정 형편이 어려워서 학비 걱정이 많았습니다.
그런데 이 장학금 덕분에 (      ㄱ      ).
이번에 회사에서 첫 월급을 받았습니다. 저도 (      ㄴ      ) 이렇게 성금을 기부합니다.
\`\`\`

**정답 예시**:
- ㄱ: 학비 걱정 없이 학교를 다닐 수 있었습니다
- ㄴ: 어려운 학생을 돕고 싶어서

---

## [52번] 설명문 빈칸 완성 (10점)
**형식**: 6~10줄의 설명문/정보 텍스트에서 2개의 빈칸(ㄱ, ㄴ)을 채우기
**글자 수**: 각 빈칸 약 30~40자 내외
**톤**: 격식체, 객관적 서술

**예시 (83회 기출)**:
\`\`\`
[동지에 대한 설명]
동지는 일 년 중 밤이 가장 길고 (      ㄱ      ) 날이다.
예로부터 한국에서는 동짓날에 팥죽을 먹는 풍습이 있다.
팥의 붉은색이 나쁜 기운을 쫓아낸다고 믿었기 때문이다.
그래서 동짓날에는 (      ㄴ      ) 집 안 곳곳에 뿌리기도 했다.
\`\`\`
...
## [53번] 그래프/도표 설명 (30점)
**형식**: 주어진 그래프, 도표, 통계 자료를 분석하고 설명하는 글쓰기
**글자 수**: 200~300자 (원고지 기준)
**구조**: 
1. 조사 개요 소개 (무엇에 대한 조사인지)
2. 주요 수치/결과 설명 (가장 높은/낮은 항목, 비율)
3. 변화 추이 또는 비교 분석 (증가/감소, 차이점)

**필수 표현**:
- "~에 대해 조사한 결과이다"
- "~이/가 가장 높은/낮은 것으로 나타났다"
- "~은/는 ~%를 차지했다"
- "반면에", "한편", "그에 비해"

**예시 주제**:
- 직장인 스트레스 원인 조사 (원형 그래프)
- 연도별 출생률 변화 (막대 그래프)
- 세대별 여가 활동 선호도 (복합 그래프)

---

## [54번] 논술형 에세이 (50점) - 가장 중요!
**형식**: 주어진 주제에 대해 자신의 의견을 논리적으로 서술하는 논술문
**글자 수**: 600~700자 (원고지 기준)
**구조**: 
1. **서론** (~100자): 주제 소개, 자신의 입장 제시
2. **본론** (~400~500자): 2~3개의 논거 + 구체적 예시/근거
3. **결론** (~100자): 요약 및 마무리

**빈출 주제 유형**:
- 찬반 논쟁: "~에 대해 어떻게 생각합니까?" (예: 재택근무 확대, SNS 영향)
- 설명형: "~의 장단점을 논하시오" (예: 온라인 교육, 1인 가구 증가)
- 원인/해결: "~의 원인과 해결방안을 쓰시오" (예: 환경오염, 세대갈등)

**필수 구성 요소**:
- 명확한 입장 표명
- 최소 2개 이상의 논거
- 구체적 예시 또는 통계 인용
- 논리적 연결 (첫째, 둘째, 마지막으로 / 그러나, 따라서, 결론적으로)

---

## ⚠️ 쓰기 문제 생성 시 필수 규칙

1. **선다형 아님**: options 필드는 빈 배열 [] 또는 ["예시 답안"]으로 설정
2. **correct_answer**: 쓰기는 주관식이므로 1로 고정 (형식상)
3. **question_text**: 문제 지시문 + 빈칸이 있는 지문(51,52) 또는 주제 제시(53,54)
4. **writing_prompt**: 별도 필드로 정확한 작문 지시사항 포함
5. **model_answer**: 모범 답안 제시 (평가 기준으로 활용)
6. **word_limit**: 글자 수 제한 명시 (53번: 200-300자, 54번: 600-700자)
...
- **TOPIK II(중급~고급)**: TOPIK II는 *기본값 자체가 어렵게* 가야 합니다.
  - (최소) **3~4급 난이도**를 기본으로 깔고 시작
  - 사회/직장/행정/뉴스/강연 주제 + 긴 문장·접속 표현·추상어/한자어/관용 포함
  - **의도·태도·뉘앙스/함의 추론**을 반드시 요구 (말 그대로/직접 정답이 튀어나오지 않게)
  - 보기 4개는 서로 의미가 *아주 비슷* 하게 만들고, **패러프레이즈(바꿔 말하기)** 로 함정 만들기
  - "키워드 매칭"으로 풀 수 없게: 스크립트 표현과 정답선지 표현을 일부러 다르게 쓰기

(그리고 beginner/intermediate/advanced는 위 시험 차이를 바탕으로 TOPIK II에서는 더 공격적으로 올리고, TOPIK I에서는 더 단순하게 낮추세요.)
